...
def precision_from_grads(program: ExactRealProgram,
                         ...
                         grads: List[Tuple[bf.BigFloat, bf.BigFloat]],
                         t: int) -> List[int]:
    ...
    # print(grads)
    # print(critical_path)
    precs = [int(50 * 1.25**(t+1)) if i in critical_path else int(50 * 1.25**t)
             for i, prec in enumerate(prev_precisions)]
    # print(critical_path)
    # print(precs)
...
def max_gradient(program: ExactRealProgram, grads: List[Tuple[bf.BigFloat, bf.BigFloat]]) -> Set[ExactRealProgram]:
    nodes = []
    ...
    reversed_grads = list(reversed([(i + 1, bf.sub(grad[0], grad[1], grad_context)) for i, grad in enumerate(grads[1:])]))
